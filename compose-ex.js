...
let averageDollarValue = _.compose(_average, map(_.prop('dollar_value')))
...
let formatPrice = _.pipe(_.prop('dollar_value'), accounting.formatMoney)
let availablePrices = _.pipe(_.filter(_.prop('in_stock')), _.map(formatPrice), _.join(', '))
...
let fastestCar = _.pipe(_.sortBy(_.prop('horsepower')), _.last, _.prop('name'), append(' is the fastest'))
